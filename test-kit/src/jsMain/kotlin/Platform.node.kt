...
internal suspend fun nodeFile(project: String, path: String): String =
    node.fs.readFile("${js("process.env")["PROJECT_ROOT"]}/$project/src/commonTest/resources/$path")
        .toString(BufferEncoding.utf8)

internal suspend fun nodeReadFile(project: String, path: String): ByteReadChannel {
    val buffer = node.fs.readFile("${js("process.env")["PROJECT_ROOT"]}/$project/src/commonTest/resources/$path")
